...
Run `quickstart.sh` script.
```shell
./quickstart.sh
```
This will create a new Python virtual environment, install the dependencies in [`requirements.txt`](requirements.txt),
and create `runtime.ini` files in both of the controllers to instruct Webots to run using this virtual environment.
